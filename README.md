...
g++ -Wall -g -std=c++0x -O3  *.cpp -o GHcaller
  Mac:
clang++ -std=c++11 -stdlib=libc++ -O3 -Wall *.cpp -o GHcaller
(for Mac, clang++ needs to be version 4+)


Description:

  Genotype-Haplotype SNP calling from mpileup data
  Input: mpileup data from STDIN, for a single chromosome, e.g. "samtools mpileup -r chr1 infile1.bam infile2.bam > data.chr1.mpileup"
  Output: fasta file
